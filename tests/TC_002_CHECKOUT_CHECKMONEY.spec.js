...
test("Validate Succesfull payment with Check/Money", async ({
  ...
  await test.step("Validate to have the option to create the account", async () => {
    await expect(page.getByRole("heading")).toContainText(
      "Create New Customer Account"
    );
  });
  await test.step("Validate that firstName and LastName where carried over the registry phase", async () => {
